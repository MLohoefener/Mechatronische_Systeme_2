% Diskreter Regelkreis RK_diskr_T12.m
...
G_Sd = c2d (G_S, T_0);
G_w_krit = K_krit / (1 + G_Sd*K_krit);

% PID-Regler nach Schwingungsversuch, Takahashi (1.2)
K_R = 0.6 * K_krit;
T_N = T_krit / 2;
T_V = T_krit / 8;

q_i = [K_R*(1+T_0/(2*T_N)+T_V/T_0) -K_R*(1-T_0/(2*T_N)+2*T_V/T_0) K_R*T_V/T_0];
p_i = [1 -1 0];
G_R = tf (q_i, p_i, T_0);
...
figure ('Name', 'Fuehrung', 'NumberTitle', 'off', 'Position', [0 100 800 600]);
  ...
  axis ([0 T_E -2 2])
  title ('Diskrete Fuehrungssprungantwort nach Takahashi (1.2)', 'fontsize', 18)
  xlabel ('Zeit [s]')
  ylabel ('h(t)')
  legend ('w(t)', 'x(t)', 'y(t)')
  legend boxoff
...
figure ('Name', 'Stoerung', 'NumberTitle', 'off', 'Position', [200 200 800 600]);
  ...
  set (findobj (gcf, 'type', 'line'), 'LineWidth', 1);
  axis ([0 T_E -1.6 1.1])
  title ('Diskrete Stoersprungantwort nach Takahashi (1.2)', 'fontsize', 18)
  xlabel ('Zeit [s]')
  ylabel ('h(t)')
  legend ('z(t)', 'x(t)', 'y(t)')
  legend boxoff
...
figure ('Name', 'Pol-Nullstellen', 'NumberTitle', 'off', 'Position', [100 0 800 600]);
  ...
  title ('Diskretes Pol-Nullstellenbild nach Takahashi (1.2)', 'fontsize', 18)
  xlabel ('Realteil')
...
figure ('Name', 'Stabilitaet', 'NumberTitle', 'off', 'Position', [300 100 800 600]);
  ...
  set (findobj (gcf, 'type', 'line'), 'LineWidth', 1);
  axis ([0 T_E -0.5 2.5])
  title ('Stabilitaetsgrenze nach Takahashi (1.2)', 'fontsize', 18)
  xlabel ('Zeit [s]')
  ylabel ('h(t)')
  legend ('w(t)', 'x(t)', 'y(t)')
  legend boxoff
