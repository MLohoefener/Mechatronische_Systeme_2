...
% Minimal-Varianz-Regler für C(z^-1)=A(z^-1) und D(z^-1)=B(z^-1)
q_i = b_i(2:end) - a_i(2:end);
p_i = b_i(2:end);
G_R = tf (q_i, p_i, T_0);
...
figure ('Name', 'Fuehrung', 'NumberTitle', 'off', 'Position', [0 100 800 600]);
  ...
  axis ([0 T_E -1.1 2.1])
  title ('Minimal-Varianz-Fuehrungssprungantwort Fuehrungsfall', 'fontsize', 18)
  xlabel ('Zeit [s]')
  ylabel ('h(t)')
  legend ('w(t)', 'x(t)', 'y(t)')
  legend boxoff
...
figure ('Name', 'Stoerung', 'NumberTitle', 'off', 'Position', [200 200 800 600]);
  ...
  set (findobj (gcf, 'type', 'line'), 'LineWidth', 1);
  axis ([0 T_E -1.1 1.1])
  title ('Minimal-Varianz-Stoersprungantwort Fuehrungsfall', 'fontsize', 18)
  xlabel ('Zeit [s]')
  ylabel ('h(t)')
  legend ('z(t)', 'x(t)', 'y(t)')
  legend boxoff
...
figure ('Name', 'Pol-Nullstellen', 'NumberTitle', 'off', 'Position', [100 0 800 600]);
  ...
  axis ([-1 1 -1 1])
  title ('Minimal-Varianz-Pol-Nullstellenbild Fuehrungsfall', 'fontsize', 18)
  xlabel ('Realteil')
