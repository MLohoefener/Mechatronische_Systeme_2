% Diskreter Regelkreis RK_diskr_T21.m
...
% PID-Regler nach Übergangsfunktion, Takahashi (2.1)
K   = 1.2*T_G / (T_U+T_0) - 0.3*T_G*T_0 / (T_U+T_0/2)^2;
T_N = K*(T_U+T_0/2)^2 / (0.6*T_G);
T_V = T_G / (2*K);
K_R = K / K_S;
...
figure ('Name', 'Fuehrung', 'NumberTitle', 'off', 'Position', [0 100 800 600]);
  ...
  axis ([0 T_E -2 2])
  title ('Diskrete Fuehrungssprungantwort nach Takahashi (2.1)', 'fontsize', 18)
  xlabel ('Zeit [s]')
  ylabel ('h(t)')
  legend ('w(t)', 'x(t)', 'y(t)')
  legend boxoff
...
figure ('Name', 'Stoerung', 'NumberTitle', 'off', 'Position', [200 200 800 600]);
  ...
  set (findobj (gcf, 'type', 'line'), 'LineWidth', 1);
  axis ([0 T_E -1.6 1.1])
  title ('Diskrete Stoersprungantwort nach Takahashi (2.1)', 'fontsize', 18)
  xlabel ('Zeit [s]')
  ylabel ('h(t)')
  legend ('z(t)', 'x(t)', 'y(t)')
  legend boxoff
...
figure ('Name', 'Pol-Nullstellen', 'NumberTitle', 'off', 'Position', [100 0 800 600]);
  ...
  title ('Diskretes Pol-Nullstellenbild nach Takahashi (2.1)', 'fontsize', 18)
  xlabel ('Realteil')
